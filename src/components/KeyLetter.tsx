...
export function KeyLetter(props: KeyLetterProps) {
  ...
  const className = cn(
    ...
    'w-8 h-8 text-sm rounded-lg md:w-12 md:h-12 md:text-lg md:rounded-xl',
    ...
    attempt?.state === LetterState.Incorrect && 'bg-gray-500 hover:bg-gray-600 text-white',
    // Default state - theme-aware background
    (!attempt || attempt.state === LetterState.Empty) && 'bg-gray-200 hover:bg-gray-300 dark:bg-gray-600 dark:hover:bg-gray-500 text-gray-700 dark:text-gray-200',
    // Disabled state
